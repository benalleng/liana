...
sender. The test talks to the *production* payjoin directory and OHTTP relay
(the same defaults lianad uses in production: `payjo.in` and the configured
ohttp relay). Network access is therefore required and the test is skipped
when `PAYJOIN_CLI_PATH` is not set.
...
def _payjoin_cli_args(datadir, bitcoind, wallet):
    """Build the global payjoin-cli flags (everything before the subcommand).

    payjoin-cli has no config file: bitcoind RPC, db path, OHTTP relay and
    pj directory are all passed as CLI flags. Match the lianad defaults so
    this test exercises the same prod payjoin infra.
    """
...
def test_payjoin_receive(lianad, bitcoind, directory):
    """End-to-end payjoin receive against the public directory + ohttp relay."""
    ...
    cli_args = _payjoin_cli_args(cli_datadir, bitcoind, SENDER_WALLET)
    ...
    try:
        ...
        #    Match by outpoint.txid rather than address text to avoid any
        #    encoding mismatch, and assert the derivation index lines up with
